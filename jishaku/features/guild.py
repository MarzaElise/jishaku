...
class GuildFeature(Feature):
    ...
    def apply_overwrites(permissions: dict, allow: int, deny: int, name: str):
        ...
        allow: diskord.Permissions = diskord.Permissions(allow)
        deny: diskord.Permissions = diskord.Permissions(deny)
    ...
    async def jsk_permtrace(
        ...
        channel: typing.Union[diskord.TextChannel, diskord.VoiceChannel],
        *targets: typing.Union[diskord.Member, diskord.Role]
        ...
        member_ids = {target.id: target for target in targets if isinstance(target, diskord.Member)}
        roles = []
        ...
        if member_ids and channel.guild.owner_id in member_ids:
            ...
            for key in dict(diskord.Permissions.all()).keys():
        ...
        embed = diskord.Embed(color=0x00FF00, description=description)
