...
UserIDConverter = commands.IDConverter[diskord.User] if diskord.version_info >= (2, 0) else commands.IDConverter
...
class SlimUserConverter(UserIDConverter):  # pylint: disable=too-few-public-methods
    ...
    async def convert(self, ctx: commands.Context, argument: str) -> diskord.User:
        """Converter method"""
        match = self._get_id_match(argument) or re.match(r'<@!?([0-9]{15,20})>$', argument)

        if match is not None:
            user_id = int(match.group(1))
            result = ctx.bot.get_user(user_id) or diskord.utils.get(ctx.message.mentions, id=user_id)
            if result is None:
                try:
...
class InvocationFeature(Feature):
    ...
    if hasattr(diskord, 'Thread'):
        OVERRIDE_SIGNATURE = typing.Union[SlimUserConverter, diskord.TextChannel, diskord.Thread]  # pylint: disable=no-member
    else:
        OVERRIDE_SIGNATURE = typing.Union[SlimUserConverter, diskord.TextChannel]
    ...
    async def jsk_override(
        ...
        *, command_string: str
        ...
        for override in overrides:
            if isinstance(override, diskord.User):
                # This is a user
                if ctx.guild:
    ...
    async def jsk_source(self, ctx: commands.Context, *, command_name: str):
        ...
        if use_file_check(ctx, len(source_text)):  # File "full content" preview limit
            await ctx.send(file=diskord.File(
                filename=filename,
