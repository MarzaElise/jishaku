...
class RootCommand(Feature):
    ...
    async def jsk(self, ctx: commands.Context):  # pylint: disable=too-many-branches
        ...
        summary = [
            f"Jishaku v{package_version('jishaku')}, diskord `{package_version('diskord')}`, "
        ...
        if isinstance(self.bot, diskord.AutoShardedClient):
    ...
    async def jsk_cancel(self, ctx: commands.Context, *, index: typing.Union[int, str]):
        ...
        if index == -1:
            ...
            task = diskord.utils.get(self.tasks, index=index)
            if task:
