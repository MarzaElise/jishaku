...
class VoiceFeature(Feature):
    ...
    async def voice_check(ctx: commands.Context):
        ...
        if not diskord.voice_client.has_nacl:
            return await ctx.send("Voice cannot be used because PyNaCl is not loaded.")

        if not diskord.opus.is_loaded():
            if hasattr(diskord.opus, '_load_default'):
                if not diskord.opus._load_default():  # pylint: disable=protected-access,no-member
    ...
    async def jsk_vc_join(self, ctx: commands.Context, *,
                          destination: typing.Union[diskord.VoiceChannel, diskord.Member] = None):
        """
        ...
        if isinstance(destination, diskord.Member):
    ...
    async def jsk_vc_volume(self, ctx: commands.Context, *, percentage: float):
        ...
        if not isinstance(source, diskord.PCMVolumeTransformer):
    ...
    async def jsk_vc_play(self, ctx: commands.Context, *, uri: str):
        ...
        voice.play(diskord.PCMVolumeTransformer(diskord.FFmpegPCMAudio(uri)))
