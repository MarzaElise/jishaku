...
def executor_function(sync_function: typing.Callable):
    ...
    .. code-block:: python3
        ...
        @executor_function
        def color_processing(color: diskord.Color):
        ...
        @bot.command()
        async def color(ctx: commands.Context, color: diskord.Color=None):
            color = color or ctx.author.color
            buff = await color_processing(color=color)

            await ctx.send(file=diskord.File(fp=buff, filename='color.png'))
    """
